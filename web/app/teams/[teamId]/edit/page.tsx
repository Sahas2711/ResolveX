...
export default function EditTeamPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (res.status === 404) { setLoadError("Team not found"); return; }
      ...
      } catch { setLoadError("Failed to load team"); } finally { setIsLoadingTeam(false); }
    })();
  ...
  function validate(): FieldErrors {
    ...
    if (!trimmedName) errors.name = "Team name is required";
    else if (trimmedName.length > 100) errors.name = "Team name must be at most 100 characters";
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setServerError({
          ...
          message: data?.error?.message ?? (res.status === 409 ? "A team with this name already exists." : "Failed to update team."),
        });
      ...
      <main className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden px-4">
        ...
        <div className="animate-slide-up flex flex-col items-center gap-8 text-center">
          ...
          <div className="max-w-xs">
            <h1 className="font-serif text-3xl italic leading-tight text-aurora">Team updated.</h1>
            <p className="mt-3 text-sm leading-relaxed text-solvent/50">Changes to &ldquo;{name.trim()}&rdquo; have been saved.</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <button onClick={() => router.push(`/teams/${teamId}`)} className="btn-phosphor rounded-full px-6 py-3 text-sm">View team</button>
            <button onClick={() => router.push("/teams")} className="btn-phosphor rounded-full px-6 py-3 text-sm"
    ...
    <main className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden px-4 py-12">
      ...
      >
        ...
        <div className="mb-8 text-center">
          <h1 className="text-2xl font-medium tracking-tight text-solvent sm:text-3xl">Edit Team</h1>
          <p className="mt-1 text-sm text-solvent/40">Update the team name and description.</p>
        </div>

        {loadError ? (
          <div className="flex flex-col items-center gap-4 py-8 text-center">
            <p className="text-sm text-magma">{loadError}</p>
            <button onClick={() => router.push("/teams")} className="btn-phosphor rounded-full px-5 py-2 text-sm">Back to teams</button>
          </div>
        ) : isLoadingTeam ? (
